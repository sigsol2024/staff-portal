...
$stmt = $pdo->prepare("UPDATE staff SET status = 'suspended', updated_at = NOW() WHERE id = ?");
$stmt->execute([$id]);

$stmt = $pdo->prepare("INSERT INTO activity_logs (admin_id, action, staff_id) VALUES (?, 'suspend', ?)");
$stmt->execute([current_admin_id(), $id]);

require_once __DIR__ . '/../includes/functions.php';
set_flash('success', 'Staff suspended.');
header('Location: ' . BASE_URL . '/admin/staff-list.php');
exit;
